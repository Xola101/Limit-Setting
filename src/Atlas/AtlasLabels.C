...
void ATLASLabelOld(Double_t x,Double_t y,bool Preliminary,Color_t color, const char* txt="Internal", float tsize=-1.0) 
{
  TLatex l; //l.SetTextAlign(12);
  l.SetNDC();
  if (tsize>=0.0) l.SetTextSize(tsize); 
  l.SetTextFont(42);
  l.SetTextColor(color);
  TString ltxt= "#font[72]{ATLAS}";
  if (Preliminary) {
    ltxt.Append(" ");
    ltxt.Append(txt);
  }
  l.DrawLatex(x,y,ltxt);
}
